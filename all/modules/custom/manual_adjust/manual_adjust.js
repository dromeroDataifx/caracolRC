...
ManualCrop.init = function() {
  ...
  for (var identifier in fields) {
    for (var k in fields[identifier].required) {
      $('.manual_adjust-identifier-' + identifier + ' option[value="' + fields[identifier].required[k] + '"]')
        .addClass('manual_adjust-style-required');
    }
  }

  $('.manual_adjust-cropdata').trigger('change');
}
...
ManualCrop.afterUpload = function(context) {
  ...
  for (var identifier in fields) {
    if (fields[identifier].instantCrop) {
      if ($('.manual_adjust-cropdata', context).length == 1) {
        $('.manual_adjust-style-button, .manual_adjust-style-thumb', context).trigger('mousedown');
      }
    }
  }
}
...
ManualCrop.showCroptool = function(identifier, style, fid) {
  ...
  $('.manual_adjust-file-' + fid + '-holder img').imagesLoaded(function() {
    ...
    if (!ManualCrop.croptool) {
      ...
      if ($('#manual_adjust-overlay-' + fid).length == 1) {
        cropType = 'overlay';
        origContainer = $('#manual_adjust-overlay-' + fid);
      }
      else {
        cropType = 'inline';
        origContainer = $('#manual_adjust-inline-' + fid);
      }
      ...
      ManualCrop.output = $('#manual_adjust-area-' + fid + '-' + styleName);
      ...
      var image = $('.manual_adjust-image', origContainer);
      ...
      image = $('.manual_adjust-image', ManualCrop.croptool)
      ...
      if (settings) {
        switch (settings.effect) {
          ...
          case 'manual_adjust_crop_and_scale':
      ...
      $('.manual_adjust-image-style', ManualCrop.croptool).text(styleName);
      ...
      if (typeof styleSelect != 'undefined') {
        ...
        $('.manual_adjust-style-button-' + fid).hide();
      }
      ...
      var instantPreview = $('.manual_adjust-instantpreview', ManualCrop.croptool);
...
ManualCrop.closeCroptool = function(reset) {
  if (ManualCrop.croptool) {
    ...
    $('.manual_adjust-style-button').show();
...
ManualCrop.resetSelection = function() {
  if (ManualCrop.croptool) {
    if (ManualCrop.oldSelection) {
      ...
      // Hide reset button.
      $('.manual_adjust-reset', ManualCrop.croptool).hide();
    }
    else {
...
ManualCrop.maximizeSelection = function() {
  if (ManualCrop.croptool) {
    var image = $('img.manual_adjust-image', ManualCrop.croptool);
...
ManualCrop.clearSelection = function() {
  if (ManualCrop.croptool) {
    ...
    // Hide clear button.
    $('.manual_adjust-clear', ManualCrop.croptool).hide();
  }
}
...
ManualCrop.updateSelection = function(image, selection) {
  if (ManualCrop.croptool) {
    ...
    image = $('img.manual_adjust-image', ManualCrop.croptool);
    ...
    if (selection && selection.width && selection.height && selection.x1 >= 0 && selection.y1 >= 0) {
      ...
      $('.manual_adjust-selection-x', ManualCrop.croptool).text(selection.x1);
      $('.manual_adjust-selection-y', ManualCrop.croptool).text(selection.y1);
      $('.manual_adjust-selection-width', ManualCrop.croptool).text(selection.width);
      $('.manual_adjust-selection-height', ManualCrop.croptool).text(selection.height);
      ...
      $('.manual_adjust-selection-x', ManualCrop.croptool).text('-');
      $('.manual_adjust-selection-y', ManualCrop.croptool).text('-');
      $('.manual_adjust-selection-width', ManualCrop.croptool).text('-');
      $('.manual_adjust-selection-height', ManualCrop.croptool).text('-');
    ...
    if (ManualCrop.oldSelection) {
      // Show reset button.
      $('.manual_adjust-reset', ManualCrop.croptool).show();
    }

    // Hide clear button.
    $('.manual_adjust-clear', ManualCrop.croptool).show();
  }
}
...
ManualCrop.selectionStored = function(element, fid, styleName) {
  ...
  $('.manual_adjust-file-' + fid + '-holder img').imagesLoaded(function() {
    var previewHolder = $('.manual_adjust-preview-' + fid + '-' + styleName + ' .manual_adjust-preview-cropped');
    if (!previewHolder.length) {
      previewHolder = $('.manual_adjust-preview-' + fid + ' .manual_adjust-preview-cropped');
    }

    var defaultPreview = $('.manual_adjust-preview-' + fid + '-' + styleName + ' > img');
    if (!defaultPreview.length) {
      defaultPreview = $('.manual_adjust-preview-' + fid + ' > img');
    }

    var toolOpener = $('.manual_adjust-style-select-' + fid + " option[value='" + styleName + "'], .manual_adjust-style-button-" + fid + ', .manual_adjust-style-thumb-' + fid + '-' + styleName + ' .manual_adjust-style-thumb-label');
    var hasClass = toolOpener.hasClass('manual_adjust-style-cropped');
    ...
    if (selection) {
      if (previewHolder.length) {
        ...
        if (maxWidth > 0) {
          ...
          var originalImage = $('#manual_adjust-overlay-' + fid + ' img.manual_adjust-image, #manual_adjust-inline-' + fid + ' img.manual_adjust-image');
      ...
      if (!hasClass) {
        ...
        toolOpener.addClass('manual_adjust-style-cropped');
      ...
      // Style not cropped.
      toolOpener.removeClass('manual_adjust-style-cropped');
...
$(document).ready(function() {
  ...
  $('.manual_adjust-button').live('mousedown', function() {
    this.blur();
  });
  ...
  Drupal.behaviors.manualCrop = {
    attach: function(context, settings) {
      ...
      $('.ajax-new-content', context).once('manual_adjust', function() {
        var element = $(this);
